...
from app.mcp.auth import MCPAuthError, _api_keys_match, resolve_auth_context
...
def test_api_keys_match_is_timing_safe():
    assert _api_keys_match("secret-key", "secret-key") is True
    assert _api_keys_match("secret-key", "wrong-key!") is False
    assert _api_keys_match("short", "much-longer-secret") is False
    assert _api_keys_match("", "secret") is False
