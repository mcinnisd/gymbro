...
and mounts it under Flask at ``/api/mcp`` so local Flask (:5001) + a tunnel
works for phone / Grokbot without migrating to FastAPI.
"""
...
def _transport_security() -> TransportSecuritySettings:
    """
    DNS-rebinding protection is off by default so Cloudflare Tunnel / ngrok
    hostnames work. Auth is enforced separately on every request.
